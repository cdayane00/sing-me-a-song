...
describe('Integrantion tests', () => {
  ...
  describe('GET /recommendations', () => {
    ...
    it('should return the last 10 recommendations', async () => {
      const recommendations = await Promise.all(
        _.times(15, async () => createRecommendation()),
      );
      const recommendationOrderedById = _.orderBy(recommendations, ['id'], ['desc']);
      const last10Recommendations = _.take(recommendationOrderedById, 10);
      const res = await supertest(app).get('/recommendations').send();
  ...
  describe('GET /recommendations/top/:amount', () => {
    it('should return 10 recommendations ordered desc by score', async () => {
      const recommendations = await Promise.all(
        _.times(15, async () => createRecommendation({ score: faker.datatype.number({ min: 15 }) })),
      );
      const recommendationsOrderedByScore = _.orderBy(recommendations, ['score'], ['desc']);
      const top10RecommendationsOrderedByScore = _.take(recommendationsOrderedByScore, 10);
